...
def create_table():
	...
	c.execute("CREATE TABLE IF NOT EXISTS details (id integer primary key autoincrement, f_id integer, git text, image text, about text, link integer, foreign key(f_id) references users(id))")
		...
		try:
			...
			if request.method == "POST":
				...
				if data:
					...
					c.execute("INSERT INTO details(f_id, git, image, about, link) VALUES(?,?,?,?,?)",(i[0],request.form['git'],pic,request.form['about'],z))
				conn.commit()
				return redirect(url_for('dash'))
			return render_template('dashboard.html',k=k,z=z,url=url,photo=photo,name=name, git=git, mob=mob,email=email)
		except:
			return render_template('hidden.html')
